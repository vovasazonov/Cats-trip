...
void OnDisable ()
    ...
    for(int i = 0; i !=32 && camera.ViewportToWorldPoint(new Vector3(0, 0, 0)).x != -2.5f; i++)             //its for animal run, if you do not need full screen , you can delete for
    {
    ...
    float texturePixelsPerWorldUnit = this.TexturePixelsPerWorldUnit + correctPixelPerUnit; //was added correctPixelPerUnit for animal run
    ...
    helpCorrectPerUnit();  //was added correctPixelPerUnit for animal run
    }
    ...
    //The value add some number to correct view of camera,
    //it's helping to hide black lines in right and left sides of phone (the changes for game animal run)
    //its for animal run, if you do not need full screen , you can delete method and forwad values
    private float correctPixelPerUnit;
    private bool cantChange;
    void helpCorrectPerUnit()
    {
        if (correctPixelPerUnit != 32f && !cantChange &&
            (Camera.main.ViewportToWorldPoint(new Vector3(0, 0, 0)).x < -2.5f ||
            Camera.main.ViewportToWorldPoint(new Vector3(0, 0, 0)).x > -2.5))
            correctPixelPerUnit++;

        else if (!cantChange && correctPixelPerUnit >= 32f && Camera.main.ViewportToWorldPoint(new Vector3(0, 0, 0)).x != -2.5f)
        {
            cantChange = true;
            correctPixelPerUnit = 0;
        }

        if (cantChange &&
            (Camera.main.ViewportToWorldPoint(new Vector3(0, 0, 0)).x < -2.6f ||
            Camera.main.ViewportToWorldPoint(new Vector3(0, 0, 0)).x > -2.4f))
            correctPixelPerUnit++;
    }
